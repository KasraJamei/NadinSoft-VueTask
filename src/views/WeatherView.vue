...
const getWeatherMapping = (temp: number) => {
    ...
    if (temp > 10) return { icon: 'mdi-weather-cloudy', text: t('weather.status_mild'), color: 'blue-grey', gradient: 'to top right, #90a4ae, #78909c' };
    if (temp > 0) return { icon: 'mdi-weather-rainy', text: t('weather.status_cool_rainy'), color: 'indigo', gradient: 'to top right, #7986cb, #5c6bc0' };
    ...
    <v-container>
        ...
        <v-card class="mx-auto pa-6 elevation-8 rounded-xl" max-width="500">
            <v-card-title class="text-h5 mb-6 text-center font-weight-bold">
                {{ t('heading.find_current_weather') }}
            </v-card-title>
